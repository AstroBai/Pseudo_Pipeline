...
class GPR_Emu:
	...
	def GPRsk(self, p, alpha, n_restarts_optimizer):
		...
		#print(gp.kernel_)
		self.emulator = gp 
		#GP_AVOUT, GP_COVOUT = gp.predict(self.nodes_trial, return_cov=True)
		...
		return GP_AVOUT, GP_STDOUT, self.Process_gpkernel(gp.kernel_)
                ...
                #return GP_AVOUT, GP_COVOUT, self.Process_gpkernel(gp.kernel_)

	def predict(self, node):
		if self.emulator is not None:
			GP_AVOUT, GP_STDOUT = self.emulator.predict(node, return_std = True)
			return GP_AVOUT, GP_STDOUT
		else:
			print("Train emulator using GPRsk first")
			return
