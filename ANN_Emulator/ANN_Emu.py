...
if __name__ == "__main__":
    ...
    eps = []
    for i in range(num_points):
        abs_rel_errors = np.abs(errors[i, :] / true_values[i, :])
        eps.append(np.max(abs_rel_errors))
    abs_rel_errors = np.array(eps) * 100  # convert to percentage
    plt.hist( abs_rel_errors, bins=100, cumulative=True, density=True, histtype='step', label='ANN' )
    # read GP errors
    relative_err_gp = np.load('/home/jbai/cosmology/ann_gp/Pseudo_Pipeline/PseudoEmulator/Trial_Set/Predictions/z0.000/GPPredictions/GPPred_TrainN200_PCATrue_ScaleNodesTrue_errs.npy')
    eps_gp = []
    for i in range(num_points):
        abs_rel_errors_gp = np.abs(relative_err_gp[i, :])
        eps_gp.append(np.max(abs_rel_errors_gp))
    abs_rel_errors_gp = np.array(eps_gp) * 100  # convert to percentage
    plt.hist( abs_rel_errors_gp, bins=100, cumulative=True, density=True, histtype='step', label='GP' )
    plt.xlabel(r'Maximum Absolute Relative Error [%]')
    ...
    plt.grid()
    plt.legend()
    plt.title('Cumulative Histogram of Maximum Absolute Relative Error')
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, 'cumulative_histogram_errors.png'))
    plt.close()